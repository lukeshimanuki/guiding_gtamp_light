...
def get_n_nodes(target_dir):
    ...
    if 'two_arm' in target_dir:
        if 'n_objs_pack_1' in target_dir:
            ...
            # target_pidxs = [40064, 40071, 40077, 40078, 40080, 40083, 40088, 40097, 40098, 40003, 40007, 40012, 40018, 40020, 40023, 40030, 40032, 40033, 40036, 40038, 40047, 40055, 40059, 40060, 40062]
...
def print_epoch_test_results():
    target_dir = 'test_results/sahs_results/domain_two_arm_mover/n_objs_pack_1/qlearned_hcount_old_number_in_goal/q_config_num_train_5000_mse_weight_0.0_use_region_agnostic_True/n_mp_limit_5_n_iter_limit_2000/'
    ...
    print 'n_data {} successes {} n nodes median {} mean {} std {} n_iks {}'.format(n_data, np.mean(successes),
                                                                                    ...
                                                                                    np.std(n_nodes) * 1.96 / np.sqrt(
                                                                                        n_data), np.mean(np.hstack(pidx_iks.values())))
    seed = '2_-1_-1'
    seed = '2_-1_3'
    ...
    seed = '-1_-1_5'  # best epoch: 9206; 10901? evaluated 313 epochs

    # place loading seeds: 3,4,12 we are missing sampler seed idx 2
    seed_dir = 'test_results/sahs_results/domain_two_arm_mover/n_objs_pack_1/' \
               'qlearned_hcount_old_number_in_goal/q_config_num_train_5000_mse_weight_0.0_use_region_agnostic_True/' \
               'using_learned_sampler/1000/sampler_seed_{}/'.format(seed)
    min_n_nodes = np.inf
    ...
    for epoch_dir in epoch_dir_list:
        ...
        if n_probs >= 9:  # and n_data>=36:
            if len(epoch_dir_list) == 1:
                condition = np.median(n_nodes) < np.median(min_n_nodes)
            else:
