...
class EncodedQGNNReachabilityNet(nn.Module):
    def __init__(self, edges, n_key_configs, device, n_msg_passing):
        super(EncodedQGNNReachabilityNet, self).__init__()
        ...
        self.th_lin = nn.Sequential(
            torch.nn.Conv2d(1, out_channels, kernel_size=(1, th_in_channels), stride=1),
            nn.LeakyReLU())
    ...
    def concat_at_idx(self, vertex_val_1, vertex_val_2, *argv):
        if len(argv) == 1:
            idx = argv[0]
            ...
            idx = argv[0]
            idx2 = argv[1]
            return torch.cat((vertex_val_1[:, :, idx:idx2+1][:, :, None], vertex_val_2[:, :, idx:idx2+1][:, :, None]), -1)
    ...
    def get_vertex_activations(self, vertices):
        ...
        vertex_qk_q0_thvals = self.concat_at_idx(vertex_qk_vals, vertex_q0_vals, 2, 3)
        ...
        vertex_qk_qg_thvals = self.concat_at_idx(vertex_qk_vals, vertex_qg_vals, 2, 3)
